...
# part of every disk-cache key.  Must resolve to b12x.
...
def _spec_memo_enabled() -> bool:
    raw = os.environ.get("B12X_COMPILE_SPEC_MEMO", "1")
...
def _cute_compile_memory_cache_enabled() -> bool:
    raw = os.environ.get("B12X_COMPILE_MEMORY_CACHE", "1")
    return raw.lower() not in {"0", "false", "no", ""}


def _cute_compile_memory_cache_size() -> int:
    raw = os.environ.get("B12X_COMPILE_MEMORY_CACHE_SIZE", "1024")
    try:
...
def _cute_compile_disk_cache_enabled() -> bool:
    raw = os.environ.get("B12X_COMPILE_DISK_CACHE", "1")
    return raw.lower() not in {"0", "false", "no", ""}
...
def _cute_compile_cache_dir() -> Path:
    root = os.environ.get("B12X_COMPILE_CACHE_DIR")
    if root:
        return Path(root)
...
def _compile_environment_key() -> tuple[tuple[str, str], ...]:
    ...
    operational_env_vars = {
        ...
        "B12X_COMPILE_MEMORY_CACHE_SIZE",
        "B12X_COMPILE_SPEC_MEMO",
...
def _build_compile_manifest(
    ...
    manifest: dict[str, Any] = {
        "schema": "b12x._lib.compile_manifest.v3",
        "cache_key": cache_key,
        "cache_format": cache_format,
