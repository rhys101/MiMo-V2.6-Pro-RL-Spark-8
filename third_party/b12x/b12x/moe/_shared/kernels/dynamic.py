            ...
            if batch_base >= producer_limit:
                ...
                if cutlass.const_expr(self.share_input_across_experts):
                    ...
                    if warp_idx < input_active_warps and token_idx < num_tokens:
                        ...
                        # Split materialized FC1 gathers token-indexed input,
                        # including token zero for padded M rows. Prepare every
                        # input row so negative EP routes cannot expose stale
                        # shared activation/scale data to those loads.
