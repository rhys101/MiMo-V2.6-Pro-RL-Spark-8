#!/usr/bin/env python3
"""Fixed-prompt benchmark (prompt set v1, byte-identical across boots).

Adapted for MiMo-V2.6 from tonyd2wild's DeepSeek-V4.1-Flash benchmark (MIT, see licenses/tonyd2wild-MIT.txt):
the only changes are the thinking switch (MiMo uses enable_thinking), counting reasoning_content deltas,
and the defaults for --base/--model. Prompts and metrics are unchanged so results compare with the
DeepSeek V4.1 Flash deployment.
...
def main():
    ...
    ap.add_argument("--base", default="http://127.0.0.1:30000/v1"); ap.add_argument("--model", default="mimo-v2.6-pro")
